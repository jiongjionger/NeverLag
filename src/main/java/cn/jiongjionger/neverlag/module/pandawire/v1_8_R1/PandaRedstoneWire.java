...
public class PandaRedstoneWire extends BlockRedstoneWire {
	...
	private void calculateCurrentChanges(final World world, final BlockPosition blockposition) {
		...
		while (!this.turnOff.isEmpty()) {
			...
			int oldPower = (Integer) state.get(POWER);
			this.g = false;
	...
	private boolean canBlockBePoweredFromSide(final IBlockData state, final EnumDirection side, final boolean isWire) {
		...
		if (((state.getBlock() instanceof BlockDiodeAbstract)) && (state.get(BlockDirectional.FACING) != side.opposite())) {
			if ((isWire) && ((state.getBlock() instanceof BlockRedstoneComparator)) && (((EnumDirection) state.get(BlockDirectional.FACING)).k() != side.k()) && (side.k().c())) {
				return true;
			}
			return false;
		}
	...
	private void checkSurroundingWires(final World worldIn, final BlockPosition pos) {
		...
		int ownPower = 0;
		if (state.getBlock() == this) {
			ownPower = (Integer) state.get(POWER);
		}
	...
	private int getSurroundingWirePower(final World worldIn, final BlockPosition pos) {
		int wirePower = 0;
		Iterator<EnumDirection> iterable = EnumDirectionLimit.HORIZONTAL.iterator();
		while (iterable.hasNext()) {
			BlockPosition offsetPos = pos.shift(iterable.next());
	...
	public void onPlace(final World world, final BlockPosition blockposition, final IBlockData iblockdata) {
		if (!world.isStatic) {
			e(world, blockposition, iblockdata);
			Iterator<EnumDirection> iterable = EnumDirectionLimit.VERTICAL.iterator();
			while (iterable.hasNext()) {
				world.applyPhysics(blockposition.shift(iterable.next()), this);

			}
			iterable = EnumDirectionLimit.HORIZONTAL.iterator();
			while (iterable.hasNext()) {
				b(world, blockposition.shift(iterable.next()));

			}
			iterable = EnumDirectionLimit.HORIZONTAL.iterator();
			while (iterable.hasNext()) {
				BlockPosition blockposition1 = blockposition.shift(iterable.next());
